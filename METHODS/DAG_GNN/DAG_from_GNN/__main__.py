...
DATASET_NAME = "real" #-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- remember to set this!!!
...
if CONFIG.prior:
    ...
    log_prior = torch.unsqueeze(log_prior, 0).to(device) #mps_used
    log_prior = torch.unsqueeze(log_prior, 0).to(device) #mps_used
...
prox_plus = torch.nn.Threshold(0.0, 0.0).to(device)
...
h_A_new = torch.tensor(1.0).to(device)
h_tol = CONFIG.h_tol
...
print("-=-=-=-=-=-=")
print(f"/Users/joelhogg/Documents/disertation stuff/bench_datasets/EXPERIMENTATION/RESULTS/{DATASET_NAME}/DAG_plot_{((CONFIG.data_filename).split('/')[-1])}_{timestamp}_final_adjacency_matrix.csv")
final_df.to_csv(f"/Users/joelhogg/Documents/disertation stuff/bench_datasets/EXPERIMENTATION/RESULTS/{DATASET_NAME}/DAG_plot_{((CONFIG.data_filename).split('/')[-1])[0:-4]}_{timestamp}_final_adjacency_matrix.csv", index=True)
# Draw the DAG
...
plt.draw()
timestamp = str(time.time())
plt.savefig(os.path.expanduser(f"/Users/joelhogg/Documents/disertation stuff/bench_datasets/EXPERIMENTATION/RESULTS/{DATASET_NAME}/plots/DAG_plot_{((CONFIG.data_filename).split('/')[-1])}_{timestamp}.png"), format="PNG", dpi=500)
