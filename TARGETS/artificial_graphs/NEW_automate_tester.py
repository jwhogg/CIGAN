...
with open("results_paths2.txt","r") as f:
	results_lines = f.readlines()

with open("targets_paths2.txt","r") as f:
...
zipped = list(zip(results_list,targets_list))
for (result_path,target_path) in zipped:
	print(f'working on: {result_path}')
	os.system(f'python3.9 tester.py -target_path="{target_path[0:-1]}" -result_path="{result_path[0:-1]}"')
